...
export const AnswersPage = (props) => {
      ...
      return (
        <div className="col-md-3">
          <div className="card col-md-12">
            ...
            <input type="range" list="tickmarks" min="0" max="10" id={"r" + it.testUUID + index}
                   onChange={event => {
                     setFull(it);
  ...
  const displayScored = (score) => {
    ...
    if (score != undefined) {
      return (
        <h3 className='black-text' style={{background: perc2color(score)}}>Already scored
          for {score}%</h3>
      )
    }
    return
  ...
  if (isLoaded) {
    ...
    answersArray.sort((it, other) => {
      if (it.score == undefined) return -1; else ;
